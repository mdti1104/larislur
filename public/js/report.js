$(document).ready(function() {
    ...
    items_report_table = $('#avarage_report_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '/reports/average-report',
            data: function(d) {
                var purchase_start = '';
                var purchase_end = '';
                if ($('#ir_purchase_date_filter').val()) {
        ...
        fnDrawCallback: function(oSettings) {
            __currency_convert_recursively($('#avarage_report_table'));
        },
    });
    $(document).on('change', '#ir_supplier_id, #ir_customer_id, #ir_location_id,#search_product', function(){
    ...
    // Average Details Report
    items_report_table = $('#average_details').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '/reports/average-report/details/'+$('#average_details').data('sku'),
            data: function(d) {
                var purchase_start = '';
                var purchase_end = '';
                if ($('#ir_purchase_date_filter').val()) {
        ...
        fnDrawCallback: function(oSettings) {
            $('#footer_total_pp').html(sum_table_col($('#average_details'), 'purchase_price'));
            $('#footer_total_sp').html(sum_table_col($('#average_details'), 'row_selling_price'));
            $('#footer_total_subtotal').html(
                sum_table_col($('#average_details'), 'row_subtotal')
            );
            $('#footer_total_qty').html(
                __sum_stock($('#average_details'), 'quantity')
            );

            __currency_convert_recursively($('#average_details'));
        },
    });
