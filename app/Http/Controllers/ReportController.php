...
class ReportController extends Controller
{
    ...
    public function averageReport()
    {
        ...
        ->select(
            ...
            'p.type as product_type',
            'p.name as product_name',
            'v.product_variation_id as variation_id',
            'v.name as variation_name',
            'pv.name as product_variation',
            'u.short_name as unit',
            'purchase.transaction_date as purchase_date',
            ...
            'transaction_sell_lines_purchase_lines.qty_returned',
            'bl.name as location',
             DB::raw('count(*) as total_product,AVG(purchase_price) as average'),
            'SL.sell_line_note'
        )->groupBy('sku');
            $permitted_locations = auth()->user()->permitted_locations();
            if ($permitted_locations != 'all') {
            ...
            $variation_id = request()->get('variation_id', null);
            if (!empty($variation_id)) {
                $query->where('v.product_variation_id', $variation_id);
            }
            if (!empty(request()->purchase_start) && !empty(request()->purchase_end)) {
                $start = request()->purchase_start;
                $end =  request()->purchase_end;
            ...
            return Datatables::of($query)
                ...
                ->editColumn('average', function ($row) {
                    return '<span class="display_currency average" data-currency_symbol=true data-orig-value="' . $row->average . '">' . $row->average . '</span>';
                })
                ->editColumn('sku', function ($row) {
                    $html ='<a href="' . action('ReportController@averageReportDetails', [$row->sku]) . '">' . $row->sku . '</a>';
                    return $html;
                
                })
               
                ->rawColumns([ 'average', 'sku'])
                ->make(true);
        }
        ...
        $customers = Contact::customersDropdown($business_id, false);
        $business_locations = BusinessLocation::forDropdown($business_id);
        return view('report.avarage_report')->with(compact('suppliers', 'customers', 'business_locations')); 
    }
    public function averageReportDetails($sku)
    {
        $business_id = request()->session()->get('user.business_id');
        ...
        return view('report.avarage_detail')->with(compact('sku')); 
